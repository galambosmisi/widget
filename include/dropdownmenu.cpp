...
void DropDownMenu::handle(event ev)
{
    ...
    set_ex_size();
    if(!_selected || items.size()==0) isExtended=false;
    if(ev.button == btn_left) click=true;
    if(ev.button == -btn_left) click=false;
    if(ev.type == ev_mouse && ev.button==btn_left)
    {
        //Extend
        if(!isExtended && ev.pos_x > _x &&  ev.pos_x <_x+_size_x-box_size && ev.pos_y > _y  && ev.pos_y < _y+_size_y && items.size()>1)
        {
            isExtended=true;
            first_item=0;
        }
        if(ev.pos_x > _x+_size_x-box_size &&  ev.pos_x <_x+_size_x && ev.pos_y > _y  && ev.pos_y < _y+_size_y && items.size()>1)
        {
